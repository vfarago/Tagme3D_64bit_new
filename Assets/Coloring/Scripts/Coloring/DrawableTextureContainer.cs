...
namespace SJ.MathFun
{
	...
	public class DrawableTextureContainer
	{
		...
		private class Stroke
		{
			...
			public Stroke (Color32 prevColor, Color32 currColor, int x, int y) {
		...
		/// <summary>
		/// This is for redo/undo
		/// </summary>
		//private Stack st = new Stack();
		private List<Stroke> history = new List<Stroke>();
		private const int HISTORY_CAPACITY = 10;
		private int currentIndexForHistory = -1;
		//private Color32 prevColor = WHITE;
		...
		/// <summary>
		/// Returns the prev color 
		/// author sungjin
		/// </summary>
		/// <returns>The prev color.</returns>
		private Color32 GetPrevColor (int x, int y) {
		...
		{
			bool retVal = FillAlgorithm.fillImageWithContainer (x, y, this, color, false);
			texture.Apply ();

			return retVal;
		}

		public bool  PaintBucketToolWithHistory  (int x, int y, Color32 color)
		{

			if (history.Count - 1 != currentIndexForHistory) {
				for (int i = history.Count - 1; i > currentIndexForHistory; i--) {
					history.RemoveAt (i);
				}
				//we may use history.RemoveRange() instead.
				// now history.Count - 1 == currentIndexForHistory
			}
				

			history.Add(new Stroke(GetPrevColor(x,y), color, x,y));

			currentIndexForHistory++;

			if (currentIndexForHistory  >= HISTORY_CAPACITY) {
				history.RemoveAt(0);
				currentIndexForHistory--;
			} 

			//Debug.Log ("PaintBucketToolWithHistory currentIndexForHistory:" + currentIndexForHistory);
			//Debug.Log ("PaintBucketToolWithHistory history.count:" + history.Count);


			return PaintBucketTool (x, y, color);
		}


		public void Redo () {
			if (currentIndexForHistory < history.Count - 1) {
				currentIndexForHistory++;
				Stroke stroke = history [currentIndexForHistory];
				PaintBucketTool (stroke.x, stroke.y, stroke.currColor);	
			} else {
				//Debug.Log ("Can't redo anymore");
			}
		}


		public void Undo() {
			if (currentIndexForHistory >= 0) {
				Stroke stroke = history [currentIndexForHistory];
				//Debug.Log ("Undo  stroke.x:" + stroke.x);
				//Debug.Log ("Undo  stroke.y:" + stroke.y);
				PaintBucketTool (stroke.x, stroke.y, stroke.prevColor);
				currentIndexForHistory--;
				//Debug.Log ("Undo currentIndexForHistory :" + currentIndexForHistory);
			} else {
