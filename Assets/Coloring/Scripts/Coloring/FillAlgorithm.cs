...
namespace SJ.MathFun
{
	public static class FillAlgorithm
	{
		...
		static private bool compare (Color32 c1, Color32 c2)
		{
			if (c1.b != c2.b)
				return false;
			if (c1.r != c2.r)
		...
		static public bool fillImageWithContainer (int x, int y, DrawableTextureContainer image, Color32 color, bool drawOnBlack)
		{
			...
			int count = 0;
			//sj 
			//it seems like breadth-first search . 
			Queue<imgPixel> que = new Queue<imgPixel> ();
			que.Enqueue (new imgPixel (x, y));
			do {
				if (count++ > DrawableTextureContainer.COLORING_FAILSAFE_LIMIT)
					break;
				// Take the next pixel from coloring queue
				imgPixel n = que.Dequeue ();
				...
				if (compare (image.pixels [n.x + image.offsetTable [n.y]], targetColor)) {
					...
					if (compare (image.pixels [n.x + image.offsetTable [n.y + 1]], targetColor)) {
						que.Enqueue (new imgPixel (n.x, n.y + 1));
							...
							if (n.y > 0) {
								if (compare (image.pixels [tmpX + image.offsetTable [n.y - 1]], targetColor)) {
									if (addTopRequired) {
										...
										que.Enqueue (new imgPixel (tmpX, n.y - 1));
